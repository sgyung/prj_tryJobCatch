...
function modifyProccess( id ) {
	if (confirm("공지사항을 수정 하시겠습니까?")) {
		$("#modifyId").val(id);
		$("#modifyTitle").val( $("#title").val() );
		$("#modifyContent").val( $("#content").val() );
		$("#modifyFrm").submit();
	} else {
    	return;
    }
	         ...
	         <li class="nav-item menu"><a href="#" class="nav-link active"> 
	         	<i class="bi bi-pencil-square"></i> 
	         		<i class="right fas fa-angle-left"></i>
							...
							<ul class="nav nav-treeview">
								<li class="nav-item"><a href="http://211.63.89.133/prj_tryJobCatch/noticelist.do" class="nav-link active">
										<i class="far fa-circle nav-icon"></i>
										<p>공지사항 관리</p>
								</a></li>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card">
              ...
              <form action="http://211.63.89.133/prj_tryJobCatch/notice_modify.do" method="post" name="modifyFrm" id="modifyFrm">
              	<input type="hidden" id="modifyId" name="n_id"/>
              	<input type="hidden" id="modifyTitle" name="n_title"/>
              	<input type="hidden" id="modifyContent" name="n_content"/>
              </form>
              ...
              <div class="card-body">
                <table class="table table-bordered">
                    <tr style="height: 80px;">
                      ...
                      <td><input type="text" name="title" class="inputBox" id="title" style=" width:90% ;height: 40px; margin-top: 10px; text-align: left;" value="${ noticeInfo.n_title }"/></td>
                    </tr>
                    <tr style="height: 400px; max-height: 400px">
                      <th style=" width:100px; text-align: center; vertical-align: middle;">내용</th>
                      <td> 
            			<textarea id="content" name="content" >${ noticeInfo.n_content }</textarea>
       				</td>
                    </tr>
                </table>
              </div>
          ...
          <div style="float: left; text-align: right; "> 
            <input type="button" value="수정" class="btn btn-primary" id="modify" style="width: 150px;" onclick="modifyProccess('${ noticeInfo.n_id }')">
            </div>
            </div>
           </div>
         </section>
